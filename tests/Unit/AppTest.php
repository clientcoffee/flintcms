...
PHP;
    ...
    public function testGetMimeTypeReturnsCorrectTypes(): void
    {
        ...
        $this->assertEquals('image/webp', $method->invoke($this->app, 'webp'));
        $this->assertEquals('image/svg+xml', $method->invoke($this->app, 'svg'));
        $this->assertEquals('application/pdf', $method->invoke($this->app, 'pdf'));
        $this->assertEquals('application/zip', $method->invoke($this->app, 'zip'));
        $this->assertEquals('application/gzip', $method->invoke($this->app, 'gz'));
        $this->assertEquals('application/gzip', $method->invoke($this->app, 'gzip'));
        $this->assertEquals('application/x-tar', $method->invoke($this->app, 'tar'));
        $this->assertEquals('application/x-7z-compressed', $method->invoke($this->app, '7z'));
        $this->assertEquals('application/vnd.rar', $method->invoke($this->app, 'rar'));
        $this->assertEquals('text/markdown', $method->invoke($this->app, 'md'));
        $this->assertEquals('text/markdown', $method->invoke($this->app, 'mdx'));
