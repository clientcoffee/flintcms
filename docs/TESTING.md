...
## CI Guardrails (Avoid Failing Checks)

Before opening a PR, make sure:
- Target branch is `dev` (PRs to `main` are blocked by CI).
- Update `CHANGELOG.md` for any code change (required by pr-guard).
- Run `bun run test` locally and fix failures before pushing.
- Use `bun run lint:js:fix` to clean up eslint issues.
- Keep tests updated so coverage does not fall below the CI threshold (see `.github/workflows/ci.yml`).
